...
if not __debug__:
    ...
    debug_file = os.getcwd() + "\\results\\" + str(d) + '_' + \
        str(q) + "_" + str(n) + "_debug.txt"
...
def complete_update_validset(cap, validset, d, q, n, coeff_list):
    ...
    for i in range(1, d+1):
        for comb in combinations(cap, i + 1):
            p = Process(target=mark_visible, args=(
                sm_set, list(comb), coeff_list, q, n))
            processes.append(p)
            p.start()
    ...
    for i in range(current_index, q ** n):
        ...
        if hashset[i]:
            ...
            if len(current_cap) > d:
                vs_new = update_validset(
                    current_cap, hashset, d, q, n, coeff_list=coeff_list)
            else:
                vs_new = complete_update_validset(current_cap, hashset, len(
                    current_cap), q, n, coeff_list=coeff_list)
            maximal_cap, _ = find_maximum_cap(n, q, d, current_cap=current_cap.copy(
            ), current_index=i + 1, hashset=vs_new, coeff_list=coeff_list, cache=cache, depth=depth + [i])
            current_cap.pop()
...
def save_caps(d, q, n):
    ...
    valid_set = [True] * (q ** n)
    # AVERY: not sure what exactly this method does
    coeff_list = generate_coeffs(d, q, n)
    cache = [None] * (q ** n)
    previous_sol = os.getcwd() + "\\results\\" + str(d) + '_' + \
        str(q) + "_" + str(n - 1) + ".dat"
    current_sol = os.getcwd() + "\\results\\" + str(d) + '_' + \
        str(q) + "_" + str(n) + ".dat"
    complete_log = os.getcwd() + "\\results\\" + str(d) + '_' + \
        str(q) + "_" + str(n) + "_all.dat"
    if path.exists(complete_log):
        ...
        if d == n:
            maximum_cap = [np.zeros(n, dtype=int)] + \
                [generate_basis(n, i) for i in range(n)]
            ...
            if path.exists(previous_sol):
                ...
                for i, vec in enumerate(initial_cap):
                    initial_cap[i] = np.concatenate(
                        (vec, np.zeros(1)), axis=None).astype(int)
            initial_cap = [np.zeros(n, dtype=int)]
            for i in range(n):  # AVERY: adds all basis vectors to cap, which we don't want
                initial_cap.append(generate_basis(n, i))
            print("Starting Search...")
            starter_hashset = complete_update_validset(
                initial_cap, valid_set, d, q, n, coeff_list)
            maximum_cap, maximum_caps = find_maximum_cap(
                n, q, d, current_cap=initial_cap, hashset=starter_hashset, coeff_list=coeff_list, cache=cache)
            print()
    ...
    print("{} caps of size {} were found.".format(
        len(maximum_caps), len(maximum_caps[0])))
...
if __name__ == '__main__':
    ...
    if not __debug__:
        ...
        response = "A maximum cap for d = {}, F = {}^{}, has size {} and is: {}".format(
            d, q, n, len(maximum_cap), maximum_cap)
        print(response)
    else:
        if len(sys.argv) != 2:
            if len(sys.argv) == 4:
